...
curl -X 'POST' \
  'http://localhost:8250/data-sources?dataSource=CIVIL&dataSource=WATCHLIST&withRaw=false' \
  -H 'accept: application/json; charset=UTF-8' \
  -H 'Content-Type: application/json; charset=UTF-8'

curl --location --request POST 'http://localhost:8250/bulk-data/load?dataSource=CIVIL&progressPeriod=3000' \
--header 'Content-Type: application/json' \
--data-binary '@/home/hanifa/senzing/mydata_1/cpib_demo/civil_servant.json'

curl --location --request POST 'http://localhost:8250/bulk-data/load?dataSource=WATCHLIST&progressPeriod=3000' \
--header 'Content-Type: application/json' \
